...
### Added
- **Live alerts: the recorder is loud by default.** Until now `AIRRecorder` was write-only: it signed records to a file and never ran a detector or printed anything, so instrumenting a real agent looked like nothing happened. Every recorder now prints to stderr a banner naming the chain file on its first record, each finding the moment a detector fires (with the exact `air explain` command), and a summary at interpreter exit (steps recorded, alerts by severity, `air trace` hint), so a clean run says "0 alerts" instead of saying nothing. New module `airsdk.live` (`LiveAlerts`), new `AIRRecorder(live=, live_min_severity=)` parameters, `AIR_LIVE=0` environment switch, `NO_COLOR` respected. Detectors run on every record for the first 200 records and every 10th record after that; AIR-04 (unpaired `tool_start`) is held until exit because a running tool is not a gap. `LocalAlerter.new_findings` gained `exclude=` so held-back findings are not marked seen. Every framework wrapper routes through the recorder, so OpenAI, Anthropic, Gemini, ADK, LlamaIndex, and the LangChain callback all get this with no code change.
- **`air report security-review` (beta): the enterprise security-review evidence pack.** The moment a vendor's sale is blocked is a security questionnaire asking how the AI agent is logged, whether the logs can be trusted, and whether a human can stop it. The pack answers ten such questions (logging, tamper evidence, external anchoring, declared scope, human oversight, detection coverage, data handling, attribution, periodic review, key custody) from the agent's own signed chain: a status per question (EVIDENCED / PARTIAL / NOT EVIDENCED) computed from the records, the evidence behind it, a draft answer the vendor can paste, a remediation line for anything not evidenced, and reviewer instructions to recompute every figure with `air trace` and `air verify-public`. Framework references are limited to public, checkable sources (EU AI Act Art. 12 and 14, NIST AI RMF core functions, SOC 2 CC7.2 / CC7.3, HIPAA 45 CFR 164.312, OWASP Agentic ASI01 to ASI10). The status table prints free; the full pack requires a license. New modules `airsdk.security_review`, `airsdk._security_review_assess`, `airsdk._security_review_content`.
- **`air demo` closes with the next step**: the copy-paste line that puts AIR in the user's own agent, and the `air trace` / `air watch` commands.

- **`air watch` is free in the terminal.** Seeing findings is free everywhere; routing them (Slack / PagerDuty / hosted retention) stays Pro. The 1.3.1 license gate on `air watch` is removed.
- **`AIRCallbackHandler` default log path** moved from `air-trace-<unix>.log` in the working directory to `.air/air-trace-<unix>.log`; the banner prints the path.
...
### Changed
- **Layer 4 `air handoff verify` now fails closed by default.** When a handoff record carries no issuer-signed capability-token JWT to re-verify, the CLI fails rather than passing on the jti-match plus validation-proof path alone. Opt into the sidecar-JWT deployment model with `--allow-unverified-token`. New `VerificationResult.jwt_reverified` records whether the capability-token JWT was cryptographically re-verified; the CLI surfaces a PARTIAL warning when it was not, so a green result can never hide a skipped check. The library-level `CrossAgentVerifier(require_capability_token_jwt=...)` knob defaults to permissive for backward compatibility. Detector taxonomy unchanged: 10 OWASP Agentic + 3 OWASP LLM + 3 AIR-native = 16 total.
- **Core `cryptography` floor reverted to `>=42.0.0,<47.0`** so `pip`/`pipx install projectair` resolve broadly with Ed25519 working everywhere. ML-DSA-65 (FIPS 204) now requires the new `[pqc]` extra (`pip install 'projectair[pqc]'`, cryptography>=48). ML-DSA imports are guarded across `airsdk.agdr` and `airsdk.recorder`; on older cryptography, ML-DSA operations raise a clear `RuntimeError` with an upgrade hint and Ed25519 is unaffected.
- **PyPI `Development Status` classifier promoted to `5 - Production/Stable`.**

### Added
- **Record metadata is now bound into the signature (`meta_signed`).** New records sign over step_id, timestamp, kind, and signature_algorithm in addition to prev_hash + content_hash, so a forensic timestamp or step kind cannot be altered without breaking verification. Additive and back-compatible: legacy records (no `meta_signed`) verify over prev_hash + content_hash unchanged, and the published reference anchors are unaffected. The flag is self-protecting: flipping it on a signed record invalidates the signature. AgDR schema stays v0.7.
- **`air attest` is now registered on the CLI** (NVIDIA NRAS GPU attestation, experimental). It shipped as a module in 1.1.0 but was not wired into the `air` entry point.
- **Single-use approval-token ledger.** `AIRRecorder.approve()` records each consumed token jti and rejects a replayed approval token with `ApprovalInvalidError`.
- **`GPUAttestationConfig.require_ocsp`.** When set, a missing cached OCSP reference is a verification failure rather than a skipped check, for deployments that must enforce revocation.
- **Decision provenance capture for non-deterministic model decisions.** LLM records can now carry a typed `DecisionProvenance` (model, resolved snapshot, backend `system_fingerprint`, sampling parameters, and a `LogprobsSummary`), so the signed record captures the stochastic conditions that produced a stochastic output. Wired through `instrument_openai` / `instrument_anthropic` / `instrument_gemini` / `instrument_adk`. Additive and opt-in; heavy fields (full logprobs) are opt-in so the chain stays lean. This makes the *record* faithful; it does not claim the decision is reproducible.
- **21 CFR Part 11 §11.50 electronic-signature semantics.** `HUMAN_APPROVAL` records now carry a `SignatureMeaning` (approval / review / responsibility / authorship) via `AIRRecorder.approve(..., signature_meaning=)`. New `airsdk.esignature.signature_manifestation()` renders the §11.50 manifestation (printed name, UTC time, meaning) and `is_part11_signature()` checks completeness. §11.70 signature/record linking was already provided by the chain.
- **Audit-trail review records (`StepKind.AUDIT_REVIEW`).** `AIRRecorder.record_audit_review()` writes an IdP-verified, signed, tamper-evident review of a chain range (21 CFR Part 11 §11.10(e) / EU Annex 11 §9), with an outcome and an optional reason-for-change note.
- **Capture-time PHI referencing (`CapturePolicy` + `ReferenceVault`).** `CapturePolicy.phi_safe()` replaces the plaintext of content fields with a per-record **salted** BLAKE3 digest before signing, so PHI never enters the immutable, anchored chain and the digest is neither reversible nor correlatable. The salt and plaintext live in an access-controlled, erasable `ReferenceVault`; deleting an entry makes the anchored digest permanently non-reversible (erasure). Decision provenance is not referenced, so ALCOA+ "Accurate" evidence survives PHI mode.
- **ALCOA+ data-integrity evidence report (`air report alcoa`, beta).** Maps each ALCOA+ principle to the cryptographic evidence the chain carries. Ships with an explicit scope boundary: it evidences a faithful, tamper-evident *record*, not a validated (CSV / GAMP 5) *system*. See `docs/part11-annex11-gap-assessment.md`.

### Fixed
- **Layer 4 validation-proof verification no longer no-ops.** `verify_validation_proof` now calls `RekorBackend.verify_inclusion`: the stub backend confirms the exact canonical bytes were submitted and that the anchor coordinates bind to them; the live backend fails closed on a proof missing its anchor coordinates or anchored state. Docstrings corrected to describe the proof accurately as an agent-signed, coordinate-bound attestation, with full Merkle inclusion re-verification performed by the Layer 1 `air verify-public` path.
- **`projectair.__version__`** corrected (was a stale `0.1.6`).
- **`air upgrade` pricing** aligned to the retention-metered model on the site: Free $0, Pro $45/mo, Team and Enterprise contact sales.
- **`scripts/e2e_layer1.py`** accepts the documented `--live-tsa` / `--live-rekor` flags and exits cleanly with guidance when run without network, instead of a confusing fail-open exit.
- Em dashes removed from the shipped README and CHANGELOG.
...
### Added
- **`air watch`: real-time local alerting (beta).** Customer value: after `pip install`, get alerted the moment AIR catches something in your agent, instead of a silent chain file. `air watch <log>` tails a growing chain and prints a severity-colored alert the instant any detector fires, deduplicated by `(detector_id, step_index)` so an append-only chain never re-alerts the same finding. Reusable `airsdk.alerting.LocalAlerter` core (the same logic is intended to back the recorder hook and hosted delivery). `air trace`/`air demo` now point to it. The detector taxonomy is unchanged: 10 OWASP Agentic + 3 OWASP LLM + 3 AIR-native = 16 total. 2 dedup tests in `tests/test_alerting.py`; E2E: `air demo` then `air watch <log>` surfaces live findings in under 60s. Readiness: beta (local terminal delivery; hosted Slack/email routing is roadmap).
...
- Counterfactual replay (Q4 2026 per the spec). Requires re-running the agent with mutated inputs, out of scope for v1; the foundation (the causal graph) lands here.
