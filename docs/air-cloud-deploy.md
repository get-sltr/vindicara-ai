...
- **Auth0 tenant.** The service trusts issuer `https://dev-kilt2vkudvbu75ny.us.auth0.com/`, audience `https://api.vindicara.io`, client id `GszbWqSkD65eUjv7FrRWYO4IkmGWdd4y` (`AIR_CLOUD_OIDC_*`). The API `https://api.vindicara.io` must exist in the tenant and the SPA and CLI clients must be authorized for it, or Auth0 returns opaque tokens and every exchange is 401. Add an Action that sets `https://vindicara.io/email` and `https://vindicara.io/email_verified` on access tokens; without it the service falls back to `/userinfo` on each first sign-in. Turn on account linking so Google and password logins for one person share a workspace (the service also joins on verified email).
