...
- Pricing (the site `vindicara-site/src/routes/pricing/+page.svelte` is the source of truth; `air upgrade` mirrors it): Free $0 (`air demo` + `air trace` only), Pro $25/mo (single seat; changed from $99 on 2026-09-07, Stripe price + Payment Link must be recreated to match), Team $599/mo base (5 seats + 250k actions/mo, $1.50/1k overage, +$99/seat), Enterprise talk-to-us (self-hosted / air-gapped unit in `deploy/`). The older Individual $39 / Pro $45 models are retired.
...
- Readiness labels are mandatory on every surface: `experimental` (ML-DSA-65, GPU attestation `air attest`, Layer 4 Wave 1), `beta` (`air watch`, `air report alcoa`, delegation, self-hosted Enterprise unit), `production` (key custody, IORails bridge, core chain). Pricing-page features must be at least `beta`.
